...
for i in range(len(excel)): # for every row in the excel
    ...
    excel.at[i, 'Record Time'] = time.mktime(subject['Record Time'].timetuple()) #converted to unix timetamping
...
for csv in csv_file: #for each csv
    ...
    offset = 16 *60 *60 #create offset

    hit = excel.loc[(excel['Record Time'] <= csv_record + offset) & (excel['Record Time'] >= csv_record - offset)] #search excel rows for time which is within margin of error set by offset
    if len(hit) == 0: #if no matches, ping terminal
        print('Could not find hit for')
        print(csv_record) #print the record time of the csv file which could not be found, useful during visual inspection
